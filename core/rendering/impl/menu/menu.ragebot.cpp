...
namespace rendering {
	...
	void menu::draw_ragebot( float group_w ) const
	{
		...
		const bool is_sniper = is_custom_wep
			? ( menu_weapons::weapon_sel_rage.weapon_flat_idx >= 0 &&
			    menu_weapons::weapon_sel_rage.weapon_flat_idx < static_cast< int >( cstypes::weapons::k_total_weapons ) &&
			    cstypes::weapons::k_weapons[ menu_weapons::weapon_sel_rage.weapon_flat_idx ].group_idx == 4 )
			: ( std::clamp( menu_weapons::weapon_sel_rage.group_idx, 0, 5 ) == 4 );
		...
		if ( xui::begin_child( "##ragebot_main", col_w, body_h - k_header_h, true ) )
		{
			...
			draw_animated_item( "rb_auto_scope_anim", is_sniper, 24.0f, 3.0f, [ & ]() {
