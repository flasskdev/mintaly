...
namespace features::combat {
        ...
        bool shared::ray_vs_capsule( const math::vector3& ray_origin, const math::vector3& ray_dir, const math::vector3& capsule_a, const math::vector3& capsule_b, float radius, float& out_fraction ) const
        {
                ...
                bool shared::autowalling( ) const
                {
                        auto& tls_slot = this->g_autowall_tls_slot;
                        if ( !tls_slot.is_valid( ) )
                        {
                                return false;
                        }
                        const auto* state = tls_slot.get( );
                        return state && state->active;
                }

                shared::lagcomp::record* shared::current_autowall_record( ) const
                {
                        auto& tls_slot = this->g_autowall_tls_slot;
                        if ( !tls_slot.is_valid( ) )
                        {
                        ...
                        const auto* state = tls_slot.get( );
                        return state ? state->record : nullptr;
                }
