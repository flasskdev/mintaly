...
class CosmeticAmbientHitChecks(unittest.TestCase):
    ...
    def test_onshot_uses_actual_victim_and_duration(self):
        ...
        self.assertIn('apply_config( onshot, scene_object, false, hit_alpha, false );', chams)
        self.assertIn('apply_config( base, scene_object );', chams)
        self.assertIn('std::clamp(elapsed / hit.duration, 0.0f, 1.0f)', hit)
        self.assertIn('(1.0f - t) * (1.0f - t) * (1.0f + 2.0f * t)', hit)
