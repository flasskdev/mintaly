...
namespace config {
	...
	namespace registry {
		...
		inline std::filesystem::path get_file_path(std::wstring_view name)
		{
			...
			if (file_path.extension() != L".cfg")
			{
				file_path += L".cfg";
			}
			return k_config_dir / file_path;
		}
		...
		inline std::vector<std::wstring> list()
		{
			...
			std::error_code ec;
			if (!std::filesystem::exists(k_config_dir, ec))
			{
				return names;
			}

			for (const auto& entry : std::filesystem::directory_iterator(k_config_dir, ec))
			{
