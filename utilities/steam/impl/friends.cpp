...
namespace steam {
    ...
    bool friends::initialize() {
        if (!detail::friends_interface) {
            const auto export_addr = MODULE_EXPORT("steam_api64.dll:SteamAPI_SteamFriends_v018");
            if (export_addr) {
                detail::friends_interface = memory::safe_call<std::uintptr_t>(export_addr);
            }
        }
    ...
    int friends::get_medium_friend_avatar(std::uint64_t steam_id) {
        ...
        const auto fn = MODULE_EXPORT("steam_api64.dll:SteamAPI_ISteamFriends_GetMediumFriendAvatar");
        if (!fn) return 0;
        return memory::safe_call<int>(fn, detail::friends_interface, steam_id);
    }
    ...
    bool friends::request_user_information(std::uint64_t steam_id, bool name_only) {
        ...
        const auto fn = MODULE_EXPORT("steam_api64.dll:SteamAPI_ISteamFriends_RequestUserInformation");
        if (!fn) return false;
        return memory::safe_call<bool>(fn, detail::friends_interface, steam_id, name_only);
    }
}
